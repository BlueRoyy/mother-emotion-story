...
function LeaderLine({ from, to, color, intensity }: { from: [number, number, number]; to: [number, number, number]; color: string; intensity: number }) {
  const line = useMemo(() => {
    ...
    const material = new THREE.LineBasicMaterial({ color, transparent: true, opacity: 0.28 + intensity * 0.35 })
...
function EmotionRegions({ color, brainAreas, intensity = 0.5 }: Brain3DProps) {
  ...
  return <>
    {activeNodes.map(({ area, node }) => <group key={area}>
      <mesh position={node.position} scale={node.scale} rotation={[0.15, -0.2, 0.08]}><sphereGeometry args={[1, 32, 24]} /><meshStandardMaterial color={activeColor} emissive={activeColor} emissiveIntensity={emissive} transparent opacity={innerOpacity} depthWrite={false} /></mesh>
      <mesh position={node.position} scale={[node.scale[0] * 1.16, node.scale[1] * 1.16, node.scale[2] * 1.16]} rotation={[0.15, -0.2, 0.08]}><sphereGeometry args={[1, 32, 24]} /><meshBasicMaterial color={activeColor} transparent opacity={outerOpacity} depthWrite={false} /></mesh>
      <LeaderLine from={node.position} to={node.label} color={color} intensity={intensity} />
      <Html position={node.label} center distanceFactor={8.5} className="brain-label-wrap"><div className="brain-region-label" style={{ borderColor: color }}><span style={{ background: color }} />{area}</div></Html>
      <pointLight position={node.position} color={activeColor} intensity={0.55 + intensity * 1.3} distance={2.2} />
    </group>)}
    <Sparkles count={54} speed={0.25 + intensity * 0.45} size={1.4 + intensity * 1.1} scale={[3.5, 2.5, 2.5]} color={color} />
  </>
}

export function Brain3D({ color, brainAreas, intensity = 0.5 }: Brain3DProps) {
  return <div className="brain-3d-canvas" role="img" aria-label="Interactive 3D brain showing highlighted emotional regions">
    ...
    <div className="brain-note">Educational visualization — highlighted areas are approximate.</div>
  </div>
}
