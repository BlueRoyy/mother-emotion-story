...
export default function App() {
  ...
  const speakSegment = (sceneIdx: number, segmentIdx: number, wordOffset: number) => {
    ...
    if (!segment) {
      if (shouldContinueRef.current && sceneIdx < storyScenes.length - 1) {
        window.setTimeout(() => speakSegment(sceneIdx + 1, 0, 0), 650)
      } else {
    ...
    utterance.onstart = () => {
      ...
      setActiveSpeaker(segment.role.charAt(0).toUpperCase() + segment.role.slice(1))
      setActiveWordIndex(wordOffset)
      timerRef.current = window.setInterval(() => {
        setActiveWordIndex((current) => Math.min(current + 1, wordOffset + segmentWords.length - 1))
      }, 390)
    }

    utterance.onend = () => {
      clearTimer()
      setActiveWordIndex(wordOffset + segmentWords.length - 1)
